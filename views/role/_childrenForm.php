...
<?php foreach($children as $item){if($item->name != $model->name){ ?>
	<li><?php echo Html::checkbox('PermissionRoleForm[children][' . $item->name . ']', isset($modelChildren[$item->name]), ['value' => $item->type, 'id' => $id = 'PermissionRoleForm_children_' . $item->name, ]); ?>&nbsp;&nbsp;<?php echo Html::label($F($item), $id); ?>
	</li>
<?php }} ?>
</ul>
