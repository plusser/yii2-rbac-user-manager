...
$F = function($item, $userRoleList){
    return $item->name . ' (' . (empty($item->description) ? '...' : $item->description) . ')' . (isset($userRoleList[$item->name]) ? ((is_object($UI = $userRoleList[$item->name]) AND $UI->createdAt) ? date(' [Y-m-d H:i:s]', (int) $UI->createdAt) : ' [новая]') : '');
};
